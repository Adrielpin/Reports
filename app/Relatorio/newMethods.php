...
class oldmethods {
	...
	public static function hourReportold ($user, $b, $c, $parcela_impressao, $parcela_impressao_orcamento, $parcela_impressao_rank) {
		...
		$posicao_tabela = array(array('Data', 'Posição média', array('type' => 'string', 'role' =>'annotation')));
		foreach($posicao_real as &$Row){
			array_push($posicao_tabela, array($Row[0].'h', $Row[1], number_format($Row[1],1,',','.')));
		}
		...
		foreach($total_parcela_hour as &$b){
			...
			foreach($investimento as &$a){
				if($a[0] == $b[0]){
					...
					array_push($investimento_x_parcela_hour, array($a[0].'h', $a[1], 'R$ '.number_format($a[1],2,',','.'), $v, 'R$ '.number_format((float)$v,2,',','.')));
					$t_investimento_x_parcela += $v;
				} 
			}
			...
			foreach($cliques as &$a){
				if($a[0] == $b[0]){
					$v = round(($a[1]/$b[1])*100);
					array_push($cliques_x_parcela, array($a[0].'h', (int)$a[1], number_format($a[1],0,',','.'), (int)$v , number_format($v,0,',','.')));
					$t_cliques_x_parcela += $v;
				} 
			}
			...
			foreach($impressoes as &$a){
				if($a[0] == $b[0]){
					$v = round(($a[1]/$b[1])*100);
					array_push($impressoes_x_parcela, array($a[0].'h', (int)$a[1], number_format($a[1],0,',','.'), (int)$v , number_format($v,0,',','.')));
					$t_impressoes_x_parcela += $v;
				} 
			}
			...
			foreach($conversao as &$a){
				if($a[0] == $b[0]){
					$v = round(($a[1]/$b[1])*100);
					array_push($conversao_x_parcela, array($a[0].'h', (int)$a[1], number_format($a[1],0,',','.'), (int)$v , number_format($v,0,',','.')));
					$t_conversao_x_parcela += $v;
				} 
			}
			...
			foreach($conversao_total as &$a){
				if($a[0] == $b[0]){
					$v = round(($a[1]/$b[1])*100);
					array_push($conversao_total_x_parcela, array($a[0].'h', (int)$a[1], number_format($a[1],0,',','.'), (int)$v , number_format($v,0,',','.')));
					$t_conversao_total_x_parcela += $v;
				} 
			}
		}
